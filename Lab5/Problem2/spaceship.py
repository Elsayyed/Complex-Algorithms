...
def goldensectionsearch(handle, xl, xu, errorTol, maxItr):
    medis = 1
    R = (math.sqrt(5) - 1) / 2  # Golden Ration
    eps = math.inf
    xS = 0
    iterCount = 0
    ...
    while(eps > errorTol and iterCount < maxItr): #and iterCount < maxItr
        if (y1 > y2):
...
def handle(val):
    return -spaceship(val)

#main point of entry
minAngle = goldensectionsearch(handle, -180, 180, 0.0000000000001, 100000000)
print("the angle with the minimum distance is :", minAngle)

min_d_saturn = spaceship(minAngle)
...
'''
minimum distance to saturn at that angle was 1539592342693.3574
minimum distance to saturn at that angle was 1551841430867.0857
minimum distance to saturn at that angle was 1518935318768.449
'''